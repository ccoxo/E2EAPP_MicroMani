...
from backend.core.defaults import ICF_TELEOP_STRATEGY_VERSION, default_config
...
def base_config(require_clutch: bool) -> dict[str, Any]:
    return {
        "teleop": {
            ...
            "leftAxisOutputScale": [0.20, 0.20, 1.0, 1.0, 1.0, 0.25],
...
def test_teleop_mapper_sends_continuous_six_axis_delta() -> None:
    async def run() -> None:
        ...
        assert payload["softLimitMin"] == [-100.0, -200.0, -300.0, -10.0, -20.0, -30.0]
        assert payload["softLimitMax"] == [100.0, 200.0, 300.0, 10.0, 20.0, 30.0]
        assert payload["translationVelocityUiPerSec"] == 5000.0
        assert payload["rotationVelocityUiPerSec"] == 3.0
        assert payload["translationStartVelocityUiPerSec"] == 400.0
...
def test_teleop_mapper_uses_configured_step_limits() -> None:
    async def run() -> None:
        ...
        assert payload["deltas"]["Roll"] == 0.15
...
def test_settings_migration_updates_existing_runtime_to_icf_teleop_strategy(tmp_path: Any) -> None:
    ...
    assert config["teleop"]["leftRotationScale"] == 0.15
    assert config["teleop"]["leftAxisOutputScale"] == [0.20, 0.20, 1.0, 1.0, 1.0, 0.25]
